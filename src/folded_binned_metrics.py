...
def folded_binned_metrics(time, flux, period, t0, lags_hours=(1,3,6,12,24), nbins=200):
    ...
    low_flux = np.nanpercentile(finite_med, 5.0) if finite_med.size else np.nan
    ...
    if np.isfinite(depth) and depth > 0:
        ...
        if np.any(in_half):
            ...
            dur_phase = min(0.12, dur_bins / nbins)
        else:
            dur_phase = 0.05
    else:
        dur_phase = 0.05
    ...
    for e in np.unique(epochs):
        ...
        in_e = np.abs(ph_e) <= (1.5 * dur_phase)
        depth_low = (
            np.nanpercentile(fl_e[in_e], 10.0)
            if np.any(in_e)
            else np.nanpercentile(fl_e, 10.0)
        )
        depths.append(baseline_e - depth_low)
