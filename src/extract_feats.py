...
from utils import scaling_and_metrics, interp_cdpp, compute_secondary_depth, compute_odd_even_depth_ratio, compute_ingress_egress_asymmetry, compute_secondary_depth_snr
from per_trans_stat import per_transit_stats_simple

def extract_features_from_arrays(tTime, flux, verbose=False, refine_duration=True, use_tls=True):
    ...
    feats["secondary_depth"] = compute_secondary_depth(time_arr, flux_detr_full, period, t0, duration_days)
    ...
    feats["odd_even_depth_ratio"] = compute_odd_even_depth_ratio(time_arr, flux_detr_full, period, t0, duration_days)
    feats["ingress_egress_asymmetry"] = compute_ingress_egress_asymmetry(time_arr, flux_detr_full, period, t0, duration_days)
    feats["secondary_depth_snr"] = compute_secondary_depth_snr(time_arr, flux_detr_full, period, t0, duration_days, feats["local_noise"])
...
def extract_all_features_from_missions(target, mission="Kepler", sigma_clip=5.0, verbose=True):
...
def extract_all_features_from_lightcurve(lc, verbose=False, refine_duration=True, use_tls=True):
    time = lc.time.value
    flux = lc.flux.value
    
    # Use the internal function for feature extraction
    feats = extract_features_from_arrays(time, flux, verbose=verbose, refine_duration=refine_duration, use_tls=use_tls)
